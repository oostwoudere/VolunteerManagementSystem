...
class Data extends CI_Controller {
    ...
    //------ View ------
    public function loadOpportunityVolunteerOptions () : bool {
    ...
    public function loadOpportunityOptions () : bool {
    ...
    public function loadTest() {
        return ['Success' => true, 'Message' => 'Load Test'];
    }
    //------ Add -------
...
//            $min_length = $this->config->item('min_password_length', 'ion_auth');
    ...
    public function AddVolunteerToOpportunity() : bool {
        $auth = $this->data_model->has_authorization_to($this->data_model::AUTH_ACTIONS['Edit'], $this->data_model::AUTH_ENTITIES['Opportunities']);
        if($auth['Success']){
            $response = ['Success' => false];
    ...
    //------ Edit ------
    public function EditOpportunity(): bool {
        $auth = $this->data_model->has_authorization_to($this->data_model::AUTH_ACTIONS['Edit'], $this->data_model::AUTH_ENTITIES['Opportunities']);
        if($auth['Success']){
            // Validate Form
            $response = ['Success' => false];
    ...
    public function EditVolunteer(): bool {
        $auth = $this->data_model->has_authorization_to($this->data_model::AUTH_ACTIONS['Add'], $this->data_model::AUTH_ENTITIES['Volunteers']);
        if($auth['Success']){
            // Validate Form
            $response = ['Success' => false];
...
//            $min_length = $this->config->item('min_password_length', 'ion_auth');
            ...
            if ($response['valid'] !== true) {
                ...
                if(!$this->data_model->EditTableData($this->data_model::TABLES['Users'], $user_data, $user_id)) {
                    ...
                    if(!$this->data_model->EditTableData($this->data_model::TABLES['Volunteers'], $volunteerData, $user_id)){
                        $response['Message'] = 'Failed At Database - Add Volunteer to User ';
                        $response['data'] = ['user' => $user_data, 'volunteer' => $volunteerData];
                    } else {
                        // If Volunteer Made -> Attempt File Upload
    ...
    public function DeleteOpportunity(): bool {
