...
class Ion_auth_model extends CI_Model
{
	...
	public function verify_password_sha256($pass, $db_pass, $identity) {
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, salt, password, active, last_login')
						  ->where($this->identity_column, $identity)
						  ->limit(1)
						  ->order_by('id', 'desc')
						  ->get($this->tables['users']);
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($this->verify_password_sha256($salted_password, $user->password, $identity))
			{
				if ($user->active == 0)
				{
					$this->trigger_events('post_login_unsuccessful');
